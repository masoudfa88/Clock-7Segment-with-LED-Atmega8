...
void buzzer(unsigned int timeBuzz,unsigned int timeSleep){
if(muteState1){buzzerPin=1;
delay_ms(timeBuzz);
buzzerPin=0;
if(timeSleep>0)delay_ms(timeSleep);
}}
...
void blinkNumberChangeTimer(){
    ...
    } else{if(counterBlinkNumber<blinkWhenChangeNumberTimeOn){
        counterBlinkNumber++;
    switch (seqNumber)
    {
    case 1:
...
void setBrightnessMenu(){
    ...
    {   
        volumeCheck();
        if(checkVolumeRight()){if(brightnessSet<99)brightnessSet++;}
        if(checkVolumeLeft()){if(brightnessSet>1)brightnessSet--;}             
        showString(2,brightnessSet,2);  
    }
    brightness(brightnessSet);
...
void setTimeMenu(){
    ...
    {   
        volumeCheck();
        if(checkVolumeRight()){if(hour<24)hour++;}
        if(checkVolumeLeft()){if(hour>0)hour--;}             
        showString(2,hour,1);   
    }
    resetClickState();
    blinkEditNumber(2);
    ...
    {   
        volumeCheck();
        if(checkVolumeRight()){if(minute<59)minute++;}
        if(checkVolumeLeft()){if(minute>0)minute--;}             
...
void setDateMenu(){
    ...
    {   
        volumeCheck();
        if(checkVolumeRight()){if(ym<99)ym++;}
        if(checkVolumeLeft()){if(ym>0)ym--;}             
        showString(2,ym,2);   
    }
         resetClickState();
         showString(1,'n',1);
    ...
    {   
        volumeCheck();
        if(checkVolumeRight()){if(mm<59)mm++;}
        if(checkVolumeLeft()){if(mm>0)mm--;}             
        showString(2,mm,2);   
    } 
        resetClickState();
         showString(1,'d',1);
    ...
    {   
        volumeCheck();
        if(checkVolumeRight()){if(dm<59)dm++;}
        if(checkVolumeLeft()){if(dm>0)dm--;}             
        showString(2,dm,2);   
    } 
...
void setAlarmMenu(){
   ...
   alHour1=h;alMinute1=m;
    ...
    {   switch (alarmSet)
        ...
        case 0:
        showStringReady(OFF);
        if(checkVolumeRight()){alarmSet=1;}
        if(checkVolumeLeft()){alarmSet=2;} 
        break;
        case 1:
        showStringReady(ONE);
        if(checkVolumeRight()){alarmSet=2;}
        if(checkVolumeLeft()){alarmSet=0;} 
        break;
        case 2:
        showStringReady(EVER);
        if(checkVolumeRight()){alarmSet=0;}
        if(checkVolumeLeft()){alarmSet=1;} 
        break;
        }
        volumeCheck();   
    }
    ...
    {   
        volumeCheck();
        if(checkVolumeRight()){if(alHour1<24)alHour1++;}
        if(checkVolumeLeft()){if(alHour1>0)alHour1--;}             
    ...
    {   
        volumeCheck();
        if(checkVolumeRight()){if(alMinute1<59)alMinute1++;}
        if(checkVolumeLeft()){if(alMinute1>0)alMinute1--;}             
...
void setMuteMenu(){
    ...
    {   switch (muteState)
        ...
        case 0:
        showStringReady(OFF);
        if(checkVolumeRight()){muteState=1;}
        if(checkVolumeLeft()){muteState=1;} 
        break;
        case 1:
        showStringReady(ON);
        if(checkVolumeRight()){muteState=0;}
        if(checkVolumeLeft()){muteState=0;} 
        break;
        }
        volumeCheck();   
    }
...
void fastEditBrightnessMenu(){
            dotBlink(OFF);
            if(checkVolumeRight()){fastEditBrightness(SUM);}
            if(checkVolumeLeft()){fastEditBrightness(MINUS);}  
}
void showMenu(){
    dotBlink(OFF);
    switch (menuPosition)
    ...
    case SETTIME:
        ...
        if(clickState)setTimeMenu();
        if(checkVolumeRight())menuPosition=SETDATE;         
        if(checkVolumeLeft())menuPosition=EXIT;
        break;
    case SETDATE:
        showStringReady(SHOWDATE);
        if(clickState)setDateMenu();
        if(checkVolumeRight())menuPosition=SETALARM;         
        if(checkVolumeLeft())menuPosition=SETTIME;
        break;
    case SETALARM:
        showStringReady(ALARM);
        if(clickState)setAlarmMenu();
        if(checkVolumeRight())menuPosition=BRIGHTNESS;       
        if(checkVolumeLeft())menuPosition=SETDATE;
        break;
    case BRIGHTNESS:
        showStringReady(BRIGHTNESS);
        if(clickState)setBrightnessMenu();
        if(checkVolumeRight())menuPosition=MUTE;       
        if(checkVolumeLeft())menuPosition=SETALARM;
        break;
        case MUTE:
        showStringReady(BIP);
        if(clickState)setMuteMenu();
        if(checkVolumeRight())menuPosition=EXIT;       
        if(checkVolumeLeft())menuPosition=BRIGHTNESS;
        break;
    case EXIT:
        showStringReady(EXIT);
        if(clickState)exitMenu();
        if(checkVolumeRight())menuPosition=SETTIME;
        if(checkVolumeLeft())menuPosition=MUTE;
        break;
    }
}
